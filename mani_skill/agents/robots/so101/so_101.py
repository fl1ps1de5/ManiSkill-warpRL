...
class SO101(BaseAgent):
    """SO101 5-DOF arm + 1-DOF parallel-jaw gripper."""
    uid = "so101"
    ...
    keyframes = dict(
        rest=Keyframe(
            qpos=np.array(
                [0, -1.5708, 1.5708, 0.66, -np.pi, -10 * np.pi / 180]
            ),
            pose=sapien.Pose(q=list(euler2quat(0, 0, np.pi / 2))),
        ),
        start=Keyframe(
            qpos=np.array(
                [0, 0, 0, np.pi / 2, -np.pi / 2, 60 * np.pi / 180]
            ),
            pose=sapien.Pose(q=list(euler2quat(0, 0, np.pi / 2))),
        ),
        zero=Keyframe(
    ...
    def is_static(self, threshold: float = 0.15):
